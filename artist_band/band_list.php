...
?>
        ...
        <div class="extra">
        <section id="tfheader">
            <form id="tfnewsearch" method="POST" action="band_list.php">
                <tr><td></td><td>
            <input type = "text" class="tftextinput" name="keyword" size="21" maxlength="80" value='' placeholder='input keyword' >
                ...
                <tr><td></td><td>
            <input type = "text" class="tftextinput" name="bandmember" size="21" maxlength="80" value='' placeholder='input bandmember'>
                <input type="submit" name='button' value="Bandmember" class="tfbutton">
        </form>
        </section>
<div id="sidebar">
    <?php
...
if (isset($_POST['button'])) {
    if($_POST['button'] == 'Keyword'){
        if(isset($_POST['keyword'])){
            ...
            if($search_bandkey = $mysqli->query("call get_keyword_band('$keyword')") or die($mysqli->error)){
                while($row =$search_bandkey->fetch_object()){
                    $band_result=array();
                    $band_result['baname']=$row->baname;
                    $band_result['bbio']=$row->bbio;
                    $band_array[]=$band_result;
                }
                $search_bandkey->close();
                $mysqli->next_result();
            }
        }else{
    ...
    if($_POST['button']=='Bandmember'){
        if(isset($_POST['bandmember'])){
            $bandmember = $_POST['bandmember'];
            if($search_bandmember = $mysqli->query("call get_bandmember_band('$bandmember')") or die($mysqli->error)){
                while($row =$search_bandmember->fetch_object()){
                    $band_result=array();
                    $band_result['baname']=$row->baname;
                    $band_result['bbio']=$row->bbio;
                    $band_array[]=$band_result;
                }
                $search_bandmember->close();
                $mysqli->next_result();
            }
        }else{
        echo "No bandmember input";
        }
